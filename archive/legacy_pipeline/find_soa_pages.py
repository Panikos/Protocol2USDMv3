"""
SoA Page Finder Script - Identifies Schedule of Activities pages in PDFs.

NOTE: This script is still functional but the new `extraction/soa_finder.py`
provides equivalent functionality with cleaner integration into the modular
pipeline. Consider using `main_v2.py` which includes automatic page detection.
"""
...
def llm_is_soa_page(page_text, model, prompt_content):
    ...
    # GPT-5 family and o-series models don't support temperature
    if model not in ['o3', 'o3-mini', 'o3-mini-high', 'gpt-5', 'gpt-5-mini', 'gpt-5.1', 'gpt-5.1-mini']:
        params["temperature"] = 0
    
    # GPT-5 family and o-series use max_completion_tokens instead of max_tokens
    # Use a slightly larger budget than 5 tokens to avoid truncation errors
    if model in ['o3', 'o3-mini', 'o3-mini-high', 'gpt-5', 'gpt-5-mini', 'gpt-5.1', 'gpt-5.1-mini']:
    ...
    try:
        ...
        # If GPT-5.1 fails, try Gemini 2.5 Pro once as a fallback
        if 'gpt-5.1' in model.lower() and google_api_key:
            try:
                print("[INFO] Falling back to gemini-2.5-pro for text page adjudication...", file=sys.stderr)
                return llm_is_soa_page(page_text, 'gemini-2.5-pro', prompt_content)
            except Exception as e2:
                print(f"[ERROR] Gemini fallback for text adjudication failed: {e2}", file=sys.stderr)
        return False
...
def llm_is_soa_page_image(pdf_path, page_num, model):
    ...
    try:
        if 'gemini' in model.lower():
            ...
            # GPT-5 family and o-series use max_completion_tokens instead of max_tokens
            # Use a slightly larger budget to avoid truncation errors
            if model in ['o3', 'o3-mini', 'o3-mini-high', 'gpt-5', 'gpt-5-mini', 'gpt-5.1', 'gpt-5.1-mini']:
                params['max_completion_tokens'] = 32
            else:
                params['max_tokens'] = 32
        ...
        if 'gpt-5.1' in model.lower() and google_api_key:
            try:
                print("[INFO] Falling back to gemini-2.5-pro for vision page adjudication...", file=sys.stderr)
                return llm_is_soa_page_image(pdf_path, page_num, 'gemini-2.5-pro')
            except Exception as e2:
                print(f"[ERROR] Gemini fallback for vision adjudication failed: {e2}", file=sys.stderr)
